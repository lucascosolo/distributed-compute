...
SECRET_KEYS = frozenset({
    "HF_TOKEN", "AIPOOL_OPENAI_API_KEY", "AIPOOL_OMNIROUTE_API_KEY", "AIPOOL_TOKEN",
})
...
def _provider_config_keys(provider: CatalogProvider) -> tuple[str, ...]:
    ...
    return (
        ...
        f"{provider_prefix}_USAGE_WINDOW_SECONDS", f"{provider_prefix}_TIMEOUT_SECONDS",
...
def make_server(
    ...
    *,
    ...
    def config_snapshot() -> dict[str, object]:
        ...
        for provider in catalog:
            ...
            providers.append({
                ...
                "preflight_note": provider.preflight_note,
                ...
                "timeout_seconds": value(f"{prefix}_TIMEOUT_SECONDS") or "120",
                "enabled": (
    ...
    def readiness_snapshot() -> dict[str, object]:
        ...
        for provider in catalog:
            ...
            if profile is not None:
                ...
                health_record = coordinator.store.health(profile.id)
                next_probe = float(health_record["next_probe_at"]) if health_record else 0.0
                last_failure_reason = str(health_record["last_failure_reason"] or "") if health_record else ""
            else:
                window_end = (now // window_seconds + 1) * window_seconds
                requests, tokens, state, next_probe, last_failure_reason = 0, 0, "not_loaded", 0.0, ""
            reasons = []
            if not key_present:
            ...
            if provider.preflight_status != "verified":
    ...
    def smoke_batch_plan(max_models: int = 12, cases: int = 3, state_filter: str = "") -> dict[str, object]:
        ...
        if state_filter not in {"", "quarantined"}:
            raise ValueError("state filter must be empty or quarantined")
...
async function load(){let r=await fetch('/admin/config');let c=await r.json();let health=(await (await fetch('/admin/readiness')).json()).providers;let bySlug=Object.fromEntries(health.map(p=>[p.slug,p]));c.providers.forEach(p=>Object.assign(p,bySlug[p.slug]||{}));document.querySelector('#hfmodel').value=c.settings.AIPOOL_HF_MODEL||'';document.querySelector('#endpoint').value=c.settings.AIPOOL_OPENAI_ENDPOINT||'';document.querySelector('#openmodel').value=c.settings.AIPOOL_OPENAI_MODEL||'';let groups={};for(let p of c.providers)(groups[p.provider_slug]??={name:p.provider_name,items:[]}).items.push(p);for(let g of Object.values(groups))g.items.forEach((p,i)=>p.showLimits=i===0);let catalog=Object.values(groups).sort((a,b)=>Number(a.items[0].has_api_key)-Number(b.items[0].has_api_key)).map(familyCard).join('');let findings=c.discovered_models?.length?`<section><h3>Live findings</h3><p class="status">${c.discovered_models.length} discovered models are available for review in the coordinator database. Individual model cards are intentionally hidden; use the bounded smoke-test control above.</p></section>`:'';cards.innerHTML=(catalog||'<p class="status">No API models in the catalog.</p>')+findings}
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            if path == "/admin/provider/smoke-test":
                try:
                    ...
                    if provider.preflight_status != "verified":
                        raise RuntimeError(f"provider_preflight_pending: {provider.preflight_note}")
