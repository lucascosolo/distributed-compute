...
function familyStatus(group){let states=group.items.map(p=>p.state);if(states.includes('healthy'))return 'working';if(states.some(s=>['degraded','rate_limited','auth_required','broken'].includes(s)))return 'attention';if(states.includes('quarantined'))return 'quarantined';return 'setup'}
