        ...
        for provider in catalog:
            ...
            providers.append({
                ...
                "enabled": (
                    ...
                    else bool(os.environ.get(f"{prefix}_API_KEY") or file_values.get(f"{prefix}_API_KEY"))
                ),
...
function familyCard(group){let first=group.items[0],family=first.provider_slug,hasKey=group.items.some(p=>p.has_api_key),enabled=group.items.filter(p=>p.enabled).length,detailsId='family-details-'+family;return `<article class="card family-card ${hasKey?'configured':''}"><header class="family-header"><div><h3>${esc(group.name)}</h3><span class="tag">${group.items.length} models · ${enabled} enabled</span><br>${hasKey?'<span class="key-status key-set">API key saved</span>':'<span class="setup-needed">Setup needed</span>'}</div></header><p class="meta">${hasKey?'Provider configured; model details are collapsed.':'Add one API key to configure this provider family.'}<br>${esc(first.transport)} · <a href="${esc(first.source_url)}" target="_blank" rel="noreferrer">official source</a></p>${quotaGuidance(first)}<label class="family-key">API key ${hasKey?'(saved; leave blank to preserve)':''}<input type="password" data-provider="${esc(family)}" autocomplete="new-password" data-key="${providerKey(family,'API_KEY')}" placeholder="one key for ${esc(group.name)}"></label><div class="family-actions"><button type="button" aria-expanded="false" onclick="toggleFamily(this,'${detailsId}',${group.items.length})">View model details (${group.items.length})</button></div><div id="${detailsId}" class="model-details" hidden><div class="grid">${group.items.map(card).join('')}</div></div></article>`}
