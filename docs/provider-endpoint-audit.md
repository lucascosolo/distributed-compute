...
| Grok Build | native coding-agent / CLI candidate | pending, not activated | xAI documents Grok Build as a terminal coding agent with plan/review/approve workflow. It is tracked separately from the xAI API and requires an operator-installed authenticated CLI plus an explicit wrapper; no automated browser or account access is configured. |
